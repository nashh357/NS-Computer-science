...
class QuizApp:
   ...
   def signup(self):
       ...
       try:
           ...
           self.save_user_data(user.uid, email, user_type)
   ...
   def login(self):
       ...
       try:
           ...
           # Here you should verify the password (this is a simplified example)
           messagebox.showinfo("Success", f"Logged in as {self.user_type}")
           if self.user_type == "teacher":
               self.show_teacher_dashboard(user)
           else:
               self.show_student_dashboard(user)
       except Exception as e:
           messagebox.showerror("Error", "Failed to login. Check your credentials.")
